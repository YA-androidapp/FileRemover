...
public class SelectFileDialog extends Activity implements
	...
	public void Close(DialogInterface dialog, File fileSelected) {
		((onSelectFileDialogListener) _context)
				.onFileSelected_by_SelectFileDialog(fileSelected);
		dialog.dismiss();
		_dlgThis = null;
	}
	...
	private boolean CreateFileList(String strPath) {
		...
		if (flagNotRoot == 1) {
			...
			String Name = _fileCurrent.getParentFile().getName().equals("") ? "/"
					: _fileCurrent.getParentFile().getPath();
